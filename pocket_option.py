...
def poll_updates_loop():
    """
    Poll Telegram getUpdates in a safe loop.
    - Exits gracefully if HTTP 409 Conflict is detected.
    - Persists update offset to disk to avoid reprocessing.
    - Uses a reasonable timeout and LIMIT.
    """
    ...
    normal_backoff = float(os.getenv("POLL_INTERVAL", str(POLL_INTERVAL)))

    def is_conflict_response(resp):
        try:
            data = resp.json()
            return data.get("error_code") == 409 or "Conflict" in data.get("description", "")
        except Exception:
            return False
    ...
    while True:
        try:
            ...
            resp = requests.get(f"https://api.telegram.org/bot{TELEGRAM_TOKEN}/getUpdates", params=params)
            
            if is_conflict_response(resp):
                log("⚠️ Conflict detected: another instance running. Exiting...", to_admin=True)
                sys.exit(0)
